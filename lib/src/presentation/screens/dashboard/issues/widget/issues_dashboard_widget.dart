...
class IssuesDashboardWidget extends StatefulWidget {
  final AnalysisChartByPhase analysisChartByPhase;
  final AnalysisChartBySector analysisChartBySector;
  final List<int> issueCounts;

  const IssuesDashboardWidget({
    Key? key,
  ...
  @override
  State<IssuesDashboardWidget> createState() => _IssuesDashboardWidgetState();
}

class _IssuesDashboardWidgetState extends State<IssuesDashboardWidget> {
  ...
  @override
  void didUpdateWidget(covariant IssuesDashboardWidget oldWidget) {
            ...
            DashboardCardWidget(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    _sectionTitle(S.of(context).issuesAnalysisPerSector),
                    ...
                    BarColorWidget(
                      barColors: [
                        BarColorModel(
                            title: S.of(context).lowPriority, color: ColorSchema.barGreen),
                        BarColorModel(
                            title: S.of(context).mediumPriority, color: ColorSchema.barOrange),
                        BarColorModel(title: S.of(context).highPriority, color: ColorSchema.barRed),
                      ],
                    ),
                  ],
                ),
            ...
            DashboardCardWidget(
              child: Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _sectionTitle(S.of(context).issuesAnalysisPerPhase),
                    const SizedBox(height: 22),
                    _buildLinearIndicatorItem(
                        title: S.of(context).design,
                        percent: issuesByPhasePercent(
            ...
            Row(
              children: [
                _buildCardWithImageWidget(
                  cardTitle: S.of(context).activesIssues,
                  cardCount:
                      "${widget.issueCounts.isNotEmpty ? widget.issueCounts[0] : 0}",
                  imagePath: ImagePaths.activeIssues,
                ),
                const SizedBox(
                  width: 16,
                ),
                _buildCardWithImageWidget(
                  cardTitle: S.of(context).highIssues,
                  cardCount:
                      "${widget.issueCounts.isNotEmpty ? widget.issueCounts[1] : 0}",
                  imagePath: ImagePaths.highIssue,
                )
              ],
            ),
            const SizedBox(height: 16),
  ...
  double issuesByPhasePercent(int value) {
    int maxNumber = max(
